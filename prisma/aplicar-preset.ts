...
//
// LAS DOS GUARDAS, y por qué son las DOS (ONBOARDING-APLICAR-PRESET-SCRIPT-1, condición del
...
// `confirmacionValida`/`conexionVisible` (`./aplicar-preset.test.ts`) dispararía todo el runbook
// al cargar el módulo — y como no hay `DATABASE_URL` en el entorno del test, `fallar()` mataría el
// proceso entero con `process.exit(1)` antes de correr un solo `assert`. Comparación por
// `pathToFileURL`, no por string crudo: `import.meta.url` percent-codifica espacios (este repo
// vive bajo una ruta con espacios) y `process.argv[1]` no — una comparación de strings a secas
// daría `false` incluso corriendo como entrypoint.
