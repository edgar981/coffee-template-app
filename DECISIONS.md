...
## 2026-09-16 — Qué cambia en el programa de API directa con `accepted_payment_methods`: el panel de
métodos se rehace, el aviso de desalineo encoge sin desaparecer, y el spike de sandbox se queda casi sin
pregunta que probar (`API-DIRECTA-METODOS-REHECHOS-1`)

### 0 · Por qué este asiento existe

La partición del programa de API directa vive **sólo en la figura de un censo read-only**: fuera del
libro y, por lo tanto, **incitable** — `grep -n "son nueve\|nueve slices\|partición completa"` sobre
este archivo antes de este commit no encuentra la lista de nueve slices en ningún lado; el propio
`API-DIRECTA-DECISIONES-PROGRAMA-1` (arriba, §5) ya lo dice: *"la partición completa NO se propone
acá — vive en el censo de construcción"*. Es la misma regla que este ledger fijó el mismo día para las
mediciones de spike (`API-DIRECTA-SPIKES-ASIENTO-1`, arriba): el instrumento de medición no produce
citas, y lo que mide es incitable hasta que alguien lo escribe.

**Este asiento NO aprueba la partición ni la da por cerrada.** Registra QUÉ CAMBIA de ella ahora que una
premisa que el censo daba por falsa resultó ser cierta. La partición sigue sin gatearse — decilo así
de explícito, no una vez sino cada vez que se la nombre abajo.

### 1 · El hecho medido, y la premisa falsa que desmiente

> «el endpoint nuevo del comercio devuelve accepted_payment_methods con los metodos que la cuenta
> tiene realmente habilitados, y pedir al crear un metodo que la cuenta no tiene falla con un 404
> NOT_FOUND_ERROR reconocible por programa que nombra el metodo» (MEDIDO — `API-DIRECTA-SPIKES-ASIENTO-1`,
> arriba, §1.C y §1.D)

**Lo que ese hecho DESMIENTE:** el censo de construcción que produjo la partición de nueve slices partió
de que **no hay forma de preguntarle al proveedor qué métodos tiene habilitados un comercio** — la
misma premisa que, en su momento, `API-DIRECTA-DECISIONES-ATRIBUCION-FIX-1` (arriba) ya marcó como
corregida de un lado (la lectura de documentación decía que no se podía consultar) sin que este ledger
tuviera todavía la cifra de una respuesta real del sandbox para esa pregunta exacta. `API-DIRECTA-
SPIKES-ASIENTO-1` es la primera medición con `file:line` de una respuesta real que la resuelve del
todo: **sí se puede, y además el rechazo por método no habilitado es reconocible por programa.**

### 2 · El slice del panel de métodos — cambia de forma, no de tamaño

**Como estaba:** el panel le deja al dueño declarar qué métodos de la pasarela ofrece su tienda,
eligiendo de **una lista nuestra, escrita en nuestro código**. Nada verifica que su cuenta los tenga.
**Configuración a ciegas** — el dueño puede encender un método que su cuenta no soporta y no se entera
hasta que un comprador intente pagar con él.

**Como queda:** el panel **lee del proveedor la lista real de esa cuenta** —con la llave pública del
tenant, por cabecera, como midió `API-DIRECTA-SPIKES-ASIENTO-1` §1.C sobre `GET /v1/merchants/info`—
y **le ofrece únicamente eso**. No puede encender lo que no tiene.

**Lo que eso mata de raíz, y es la mitad del valor:**
- **El catálogo deja de vivir en nuestro código.** El proveedor agrega o saca un tipo y **nosotros no
  desplegamos nada**. La lista escrita a mano habría envejecido sola — la misma clase de dato vencido
  que este ledger ya persigue en otros lados, pero acá en código y peor, **porque decide qué se le
  puede cobrar a alguien**.
- **La pregunta «¿qué tipos soportamos?» sale del slice.** No la decidimos nosotros: la responde la
  cuenta del dueño, leída en el momento de configurar.

**Lo que le agrega, y por eso no encoge:** una llamada al proveedor **desde el panel**, con su propio
modo de falla.

> **Y ese modo de falla es byte visible, así que es decisión del owner — se anota como PENDIENTE, no se
> resuelve acá:** si no se puede leer la cuenta al configurar, ¿el dueño ve la lista vacía, la lista
> anterior, o un aviso explícito de que no se pudo leer? **El precedente del checkout —si la consulta al
> proveedor falla, no se ofrece el pago en línea (`API-DIRECTA-DECISIONES-PROGRAMA-1`, arriba, §4)— NO
> se traslada solo:** allá la consecuencia de fallar es no cobrar; acá es no poder configurar. Son
> costos distintos y pueden pedir una salida distinta.

### 3 · El slice del aviso de desalineo — encoge, pero no desaparece

**Como estaba:** un slice entero — pantalla al comprador, marca en el estado crudo del proveedor, y una
entrada nueva en el catálogo de automatizaciones (`constants/automations.ts`) para avisarle al dueño.

**Como queda:** el desalineo **se previene al configurar** (§2, arriba), así que lo que sobrevive es el
**residual**: que la cuenta del proveedor cambie **después** de que el dueño ya configuró el panel. Ahí
la creación de la transacción devuelve el `404` reconocible que midió `API-DIRECTA-SPIKES-ASIENTO-1`
§1.D, **y ese error nombra el método exacto** — así que avisarle al dueño sale **más barato que antes**:
ya se sabe cuál método se cayó, sin tener que adivinarlo comparando listas.

**Y acá está la razón por la que este slice no se borra, que es lo más importante de esta sección:**

> **[SIN MEDIR] — marcador buscable por máquina. ESTO NO ES UN DATO. ES UNA PREGUNTA ABIERTA.** Si
> `accepted_payment_methods` predice el `404` de forma confiable **para CUALQUIER tipo de método, y no
> sólo para el único que se probó** (`BRE_B`, el único caso que corrió en `API-DIRECTA-SPIKES-ASIENTO-1`
> §1.D). **NADIE MIDIÓ ESTO.** No se afirma, no se asume, no se cita como hecho, no se usa para decidir:
> se MIDE. Si se da por cierto, el slice queda mal desde su premisa.

**Lo que eso obliga:** la frase «el desalineo se previene al configurar» **descansa en un predictor que
no está medido**. Una coincidencia no es un predictor. Si la lista y el rechazo se separan aunque sea en
un tipo, este slice vuelve a ser el grande de nuevo —pantalla al comprador, marca de estado, entrada de
automatización— **y lo descubriríamos con un comprador adelante**, en el peor momento posible para
descubrirlo. Queda achicado, no cerrado, y la pregunta se mide antes de apoyarse en ella para nada que
toque a un comprador real.

### 4 · El primer slice se quedó casi sin trabajo

El primero de la partición era una **prueba de extremo a extremo, desechable y detrás de una bandera**,
y existía por una sola razón: verificar que las premisas técnicas aguantaran **una llamada real** contra
el proveedor.

**Los dos spikes de sandbox ya midieron esas premisas** —la firma sirve tal cual sin reimplementarla
(`API-DIRECTA-SPIKES-ASIENTO-1` §1.A), la creación autoriza con cualquiera de las dos llaves (§1.B), y
el corte cliente/servidor para la creación queda forzado por el secreto de integridad, no por preferencia
de diseño (§1.A)—, así que el cableado desechable **se quedó sin la pregunta que justificaba escribirlo.**

**Pero no desaparece, y ésta es la parte honesta:** le queda **la mitad del webhook** — que la cadena del
proveedor cierre contra un evento REAL, no simulado. Eso **sigue sin medirse** y **no se puede medir sin
una URL pública**, tal como ya está registrado (`WOMPI-REGLAS-IMPLEMENTACION-1`, sección de Pagos en
línea (Wompi), CLAUDE.md). Ese residual queda **BLOQUEADO por esa condición**, no resuelto por este
asiento ni por ningún otro que corra en esta misma sesión sin acceso a red.

### 5 · El tamaño del programa

De la partición propuesta —que, como dice §0, vive fuera de este libro, así que lo que sigue se registra
tal como este mismo programa lo cuenta de sí mismo, sin poder verificarlo contra una lista que el ledger
no tiene—:

| | cantidad |
| --- | --- |
| slices en la propuesta | 9 |
| de ésos, Tier 1 | 9 — los nueve tocan ruta de dinero, esquema, o bytes de cliente |
| de ésos, paran para el gate del owner | 9 — ninguno es a la vez sin esquema, sin bytes de cliente y sin contrato cruzado |
| afectados por este asiento | 3 — el panel de métodos (§2, rehecho), el aviso de desalineo (§3, encogido), el spike de sandbox (§4, con menos pregunta que probar) |

**No se encontró ninguna afirmación de esta tabla que contradiga al ledger**, porque el ledger no
contiene la partición contra la que contrastarla — `grep` de los nueve nombres de slice sobre este
archivo antes de este commit da cero líneas, igual que ya dio cero para el censo entero
(`WOMPI-API-DIRECTA-CENSO-1`, § Cierre del censo de API directa, arriba). Si algún día la partición
misma entra al libro con su propio asiento, ESE es el que debe reconciliarse contra ésta si los números
difieren — no éste, que sólo la cita.

### 6 · Límites de este asiento

- **No se construye nada de API directa.** El panel sigue siendo el de la lista escrita a mano; el
  aviso de desalineo no existe; el spike de sandbox sigue siendo el primer paso pendiente. Ningún
  código de producto cambia.
- **La partición no queda aprobada por escribir este asiento.** Sigue siendo una propuesta fuera del
  libro salvo por lo que estas seis secciones registran de ella.

**GATE, los dos carriles, verde.** Este diff toca un solo archivo del ledger (`DECISIONS.md`) y ningún
test, así que nada podía cambiar en ninguno de los dos carriles.

Regla: un predictor no medido no se cita como si estuviera medido, ni siquiera para achicar un slice —
se anota SIN MEDIR y se deja como pregunta abierta hasta que algo lo mida contra el proveedor. Un slice
que se previene en configuración no elimina a su gemelo de detección: lo encoge al residual de lo que
puede cambiar DESPUÉS de configurar, y ese residual sigue vivo mientras el predictor que lo sostiene siga
sin verificarse.
